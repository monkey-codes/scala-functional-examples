...
//    b.legalNeighbors.map {case (block, move) => (block, move :: history)}.toStream
   ...
   * 
   * Filter out neighbours already explored
   */
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    val more = for {
      ...
      (neighbour, historyPlusNeighbour) <- newNeighborsOnly(neighborsWithHistory(block, history), explored)
    }yield (neighbour, historyPlusNeighbour)
